...
:::note
There is lots of other metadata in the [custom metrics](/reference/structs/custom-metrics/), which are custom pieces of JavaScript run during the crawl to extra information from the page.

Always check that before reverting to `response_body` in `requests` table which is slow, does not include the rendered DOM after JavaScript runs, and is expensive to query.
